...
class ContinuousTransitionBottleneckTokenizer(nn.Module):
    """
    Continuous composable transition bottleneck without VQ / discretization.

    RGB/DINO branch:
      E_rgb(F_t, F_future - F_t) -> transition tokens z_rgb
      D_rgb(F_t, z) -> future DINO patch deltas

    Optional pixel-depth branch:
      DepthAnything / sensor gives raw depth maps D_t, D_future.
      The depth branch uses normalized depth maps directly:
        ViT_dep(D_t, D_future - D_t) -> depth patch tokens -> E_dep -> z_dep
      z_rgb and z_dep are fused into one canonical transition token z.
      D_dep(D_t, z) -> dense normalized depth residual.
    ...
    def forward(
        ...
        # The original auxiliary losses are RGB-only. For the depth-active version,
        # keep them disabled unless you implement depth-aware composition/reencode variants.
