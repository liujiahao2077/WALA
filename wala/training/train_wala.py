...
"""
WALA’s trainer is built directly on native PyTorch + Accelerate + DeepSpeed, keeping the loop explicit and easy to hack.
Conventions:
...
# Local Modules
from wala.dataloader import build_dataloader
from wala.model.framework.base_framework import build_framework
from wala.training.trainer_utils.config_tracker import AccessTrackedConfig, wrap_config
from wala.training.trainer_utils.trainer_tools import TrainerUtils, build_param_lr_groups, normalize_dotlist_args
...
def setup_optimizer_and_scheduler(model, cfg) -> Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
    ...
    for i, group in enumerate(param_groups):
        ...
        print(f"[OPT GROUP] {i}: name={name}, lr={lr}, tensors={n_tensors}, params={n_params}")
...
class VLATrainer(TrainerUtils):
    ...
    def _log_metrics(self, metrics):
        ...
        if self.completed_steps % self.config.trainer.logging_frequency == 0 and dist.get_rank() == 0:
            ...
            logger.info(f"Step {self.completed_steps}, Info: {metrics})")
    ...
    def eval_action_model(self, step_metrics: dict = None) -> dict:
        ...
        valid_actions = [example.get("action") for example in examples if example.get("action") is not None]
        ...
        if self.accelerator.is_main_process:
            ...
            if len(valid_actions) == 0 or len(normalized_actions) == 0:
                step_metrics["mse_score"] = 0.0 
            else:
                actions_arr = np.array(valid_actions)[:, -normalized_actions.shape[-2]:, :]  # (B_valid, chunk_len, action_dim)
                num_pots = np.prod(actions_arr.shape)
                
                assert normalized_actions.shape == actions_arr.shape, \
                    f"Eval alignment error! Pred shape: {normalized_actions.shape}, GT shape: {actions_arr.shape}"
                
                score = TrainerUtils.euclidean_distance(normalized_actions, actions_arr)
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        with self.accelerator.accumulate(self.model):
            ...
            self.optimizer.step()
            # Keep the learning-rate schedule aligned with real optimizer updates.
            if self.accelerator.sync_gradients:
                self.lr_scheduler.step()

                unwrapped_model = self.accelerator.unwrap_model(self.model)
                if hasattr(unwrapped_model, "update_ema_targets"):
                    unwrapped_model.update_ema_targets(momentum=0.996)

        return {**{k: v.detach().item() for k, v in output_dict.items()}}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_yaml",
        type=str,
        default="wala/config/training/wala_cotrain_oxe.yaml",
