...
from wala.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from wala.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
from wala.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps
...
def _detect_lerobot_version(dataset_path: Path) -> str:
    ...
    if info_path.exists():
        try:
            ...
            print(f"Warning: failed to read {info_path} for version detection: {exc}")
    ...
    raise ValueError(f"Cannot detect LeRobot version for dataset: {dataset_path}")


def _normalize_lerobot_version(version: str | None) -> str:
    if version is None:
        return "auto"
...
def _normalize_action_mode_keep_abs_indices(
    action_mode_keep_abs_indices: dict[str, Sequence[int]] | None,
) -> dict[str, list[int]]:
    normalized = {}
    for action_key, indices in (action_mode_keep_abs_indices or {}).items():
        action_key = str(action_key)
        if not action_key.startswith("action."):
            action_key = f"action.{action_key}"
        ...
        if indices is None:
            ...
            index_list = [int(item) for item in indices.replace(",", " ").split()]
        else:
            index_list = [int(item) for item in indices]

        if any(index < 0 for index in index_list):
            raise ValueError(f"action_mode_keep_abs_indices for {action_key} must be non-negative: {index_list}")
        normalized[action_key] = sorted(set(index_list))
    return normalized


def _restore_keep_abs_action_dims(
    action_key: str,
    transformed_values: np.ndarray,
    original_values: np.ndarray,
    action_mode_keep_abs_indices: dict[str, list[int]] | None,
) -> np.ndarray:
    keep_abs_indices = (action_mode_keep_abs_indices or {}).get(action_key, [])
    if not keep_abs_indices:
        return transformed_values

    width = transformed_values.shape[1]
    out_of_range = [index for index in keep_abs_indices if index >= width]
    if out_of_range:
        raise ValueError(
            f"action_mode_keep_abs_indices for {action_key} exceeds action width {width}: {out_of_range}"
        )

    transformed_values = transformed_values.copy()
    transformed_values[:, keep_abs_indices] = original_values[:, keep_abs_indices]
    return transformed_values
...
def _build_stats_cache_config(
    ...
    action_mode_keep_abs_indices: dict[str, list[int]] | None = None,
) -> dict:
    config = {"mode": action_mode}
    if action_mode_apply_keys:
        config["apply_keys"] = action_mode_apply_keys
    if action_mode_state_map:
        config["state_map"] = action_mode_state_map
    if action_mode_keep_abs_indices:
        config["keep_abs_indices"] = action_mode_keep_abs_indices
    return config
...
def _compute_statistics_for_mode(
    ...
    if action_mode == "delta":
        return calculate_delta_action_statistics(
            ...
            action_mode_keep_abs_indices=action_mode_keep_abs_indices,
...
def calculate_delta_action_statistics(
    ...
    action_mode_keep_abs_indices = _normalize_action_mode_keep_abs_indices(action_mode_keep_abs_indices)
    ...
    for parquet_path in tqdm(sorted(list(parquet_paths)), desc="Collecting delta action stats"):
        ...
        for action_col, slice_list in action_col_slices.items():
            ...
            for a_slice, state_col, s_slice, action_padding, state_padding in slice_list:
                ...
                action_key = None
                for candidate_key in _normalize_action_mode_apply_keys(action_mode_apply_keys, action_keys_full):
                    candidate_subkey = candidate_key.replace("action.", "", 1)
                    candidate_cfg = lerobot_modality_meta.action[candidate_subkey]
                    candidate_col = candidate_cfg.original_key or candidate_subkey
                    if candidate_col == action_col and (candidate_cfg.start, candidate_cfg.end) == a_slice:
                        action_key = candidate_key
                        break
                if action_key is None:
                    raise ValueError(f"Unable to resolve action key for {action_col}:{a_slice}")
            ...
            for base_index in range(trajectory_length):
                ...
                for action_key, a_slice, state_part_full, state_padding in prepared_slices:
                    ...
                    out = _restore_keep_abs_action_dims(
                        action_key,
                        transformed_values=out,
                        original_values=action_part_chunk,
                        action_mode_keep_abs_indices=action_mode_keep_abs_indices,
                    )
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        explicit_version = kwargs.get("lerobot_version", None)
        cfg_version = self.data_cfg.get("lerobot_version", "auto") if self.data_cfg else "auto"
        lerobot_version = _normalize_lerobot_version(explicit_version if explicit_version is not None else cfg_version)
        self._lerobot_version = (
            _detect_lerobot_version(self._dataset_path) if lerobot_version == "auto" else lerobot_version
        )

        self._action_mode = None
        self._action_mode_state_map = {}
        self._action_mode_apply_keys = None
        self._action_mode_keep_abs_indices = {}
    ...
    def _get_dataset_cache_dir(self) -> Path:
        cache_root = None
        if self.data_cfg:
            cache_root = self.data_cfg.get("dataset_cache_dir", None)
        cache_root = cache_root or os.environ.get("WALA_DATASET_CACHE_DIR")
        if cache_root is None:
            cache_root = Path.cwd() / "cache" / "datasets"

        dataset_id = hashlib.md5(str(self.dataset_path.resolve()).encode()).hexdigest()[:12]
        return Path(cache_root) / self.dataset_name / dataset_id

    def _get_dataset_cache_file(self, filename: str) -> Path:
        return self._get_dataset_cache_dir() / filename
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        configured_state_action_keys = {}
        for modality in ["state", "action"]:
            modality_cfg = self.modality_configs.get(modality)
            modality_prefix = f"{modality}."
            configured_state_action_keys[modality] = {
                key.replace(modality_prefix, "", 1)
                for key in (modality_cfg.modality_keys if modality_cfg else [])
        ...
        for modality in ["state", "action"]:
            ...
            for subkey in le_state_action_meta:
                if subkey not in configured_state_action_keys[modality]:
                    continue
        ...
        for new_key in le_modality_meta.video:
            ...
            # Different LeRobot dataset versions store video metadata under different keys.
            try:
        ...
        stats_path = self._get_dataset_cache_file("stats_gr00t.json")
        ...
        keep_abs_indices = _normalize_action_mode_keep_abs_indices(
            self.data_cfg.get("action_mode_keep_abs_indices", {}) if self.data_cfg else {}
        )
        stats_cache_config = _build_stats_cache_config(
            action_mode=action_mode,
            action_mode_apply_keys=apply_keys if action_mode != "abs" else None,
            action_mode_state_map=normalized_state_map if action_mode != "abs" else None,
            action_mode_keep_abs_indices=keep_abs_indices if action_mode != "abs" else None,
        )
        parquet_files = list(self.dataset_path.glob(LE_ROBOT_DATA_FILENAME))
        parquet_files_filtered = [pf for pf in parquet_files if "episode_033675.parquet" not in pf.name]
        needs_low_dim_stats = bool(action_keys_full or state_keys_full)

        if is_main():
            if needs_low_dim_stats:
                le_statistics = _load_or_compute_statistics(
                    ...
                    action_mode_keep_abs_indices=keep_abs_indices,
                )
            else:
                le_statistics = {}
        else:
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        steps_path = self._get_dataset_cache_file(steps_filename)
        ...
        if steps_path.exists():
            try:
                ...
                print(
                    ...
                    f"Cached steps config mismatch for {steps_path}, will rebuild."
                )
            except Exception as e:
    ...
    def _get_all_steps_single_process(self) -> list[tuple[int, int]]:
        """Collect all dataset steps in a single process."""
    ...
    def _get_position_and_gripper_values(self, data: pd.DataFrame) -> tuple[list, list]:
        ...
        if delta_position_values is None:
            # Use direct column names when metadata fields are unavailable
    ...
    def _init_action_mode(self) -> None:
        ...
        self._action_mode_keep_abs_indices = _normalize_action_mode_keep_abs_indices(
            self.data_cfg.get("action_mode_keep_abs_indices", {}) or {}
        )
    ...
    def _apply_action_mode(self, data: dict) -> dict:
        ...
        for action_key in action_keys:
            ...
            out = _restore_keep_abs_action_dims(
                action_key,
                transformed_values=out,
                original_values=action_values,
                action_mode_keep_abs_indices=self._action_mode_keep_abs_indices,
            )
    ...
    def _get_tasks(self) -> pd.DataFrame:
        ...
        if self._lerobot_version == "v2.0":
            ...
            if "task_index" not in df.columns:
                raise ValueError(f"v3 tasks file missing task_index: {tasks_path}")
            return df[["task_index", "task"]].set_index("task_index")
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        for video_key in video_keys:
            delta_indices = self.delta_indices[video_key]
            ...
            for i, delta in enumerate(delta_indices):
                ...
                if delta <= 0:
                    past_curr_images.append(image)
                else:
                    future_images.append(image)
                
            
            all_images.append([past_curr_images, future_images])
...
class LeRobotMixtureDataset(Dataset):
    ...
    def merge_metadata(
        ...
        for modality, configs in modality_configs.items():
            # ------------------------------------------------------------------
            # Video metadata can differ in raw resolution/fps across datasets.
            # This is OK because images are resized later before entering the model.
            # For video, only require that the view names are consistent.
            # ------------------------------------------------------------------
            if modality == "video":
                parsed_configs = [json.loads(c) for c in configs]
                ...
                assert len(key_sets) == 1, (
                    f"Multiple video key sets for modality video: {key_sets}. "
                    f"All datasets under the same embodiment tag must expose the same video keys."
                )

                # Deterministically keep one video metadata config.
                # resolution/fps may be different, but they are not used for action/state normalization.
                merged_metadata["modalities"][modality] = json.loads(sorted(configs)[0])
                continue

            # For state/action, configs must still be exactly the same.
    ...
    def update_metadata(self, metadata_config: dict, cached_statistics_path: Path | str | None = None) -> None:
        ...
        for dataset, weight in zip(self.datasets, self.dataset_sampling_weights):
            ...
            all_weights[dataset.tag].append(weight)
            
        for tag in all_metadatas.keys():
            self.merged_metadata[tag] = self.merge_metadata(
                metadatas=all_metadatas[tag],
