...
Environment variables (lower priority than flags):
  ...
  ROBOTWIN_WALA_ENV       Conda env name for the policy server (default: wala)
  ROBOTWIN_ENV               Conda env name for RoboTwin eval (default: robotwin)
  WALA_PYTHON             Explicit python path for wala (skips conda env lookup)
...
port_in_use() {
    ...
    if command -v ss >/dev/null 2>&1; then
        ...
        "${WALA_PYTHON:-python}" -c "import socket; s=socket.socket(); s.settimeout(1); s.connect(('127.0.0.1',${port})); s.close()" 2>/dev/null
    fi
}
...
check_port_detection() {
    ...
    if "${WALA_PYTHON:-python}" -c "import socket" 2>/dev/null; then
...
find_conda_python() {
    ...
    echo "  Set WALA_PYTHON / ROBOTWIN_PYTHON to the full python path instead." >&2
    return 1
}
...
prepare_runtime_dependencies() {
    ...
    "${WALA_PYTHON}" -m pip install snntorch
...
launch_task_in_slot() {
    ...
    (
        ...
        export WALA_PYTHON="${WALA_PYTHON}"
...
WALA_PYTHON="$(resolve_python "${WALA_PYTHON:-}" "${ROBOTWIN_WALA_ENV:-wala}")"
ROBOTWIN_PYTHON="$(resolve_python "${ROBOTWIN_PYTHON:-}" "${ROBOTWIN_ENV:-RoboTwin}")"
export WALA_PYTHON ROBOTWIN_PYTHON

echo "[INFO] wala python: ${WALA_PYTHON}"
