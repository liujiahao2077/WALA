...
class SimulationInferenceEnv:
    ...
    def _get_env_success(env_infos: Dict[str, Any], env_idx: int, n_envs: int) -> bool:
        ...
        success_info = np.asarray(env_infos.get("success", False))
        if success_info.ndim == 0:
            return bool(success_info)
        if success_info.shape[0] == n_envs:
            return bool(np.any(success_info[env_idx]))
        return bool(np.any(success_info))
