...
# cd ./wala
wala_PYTHON=/mnt/data/ljh/miniconda3/envs/wala/bin/python
ROBOCASA_PYTHON=/mnt/data/ljh/miniconda3/envs/robocasa/bin/python
...
if [[ -z "${CKPT_PATH}" ]]; then
    echo "Please set CKPT_DEFAULT in examples/Robocasa_tabletop/eval_files/batch_eval_args.sh" >&2
    exit 1
fi
N_ENVS=$N_ENVS_DEFAULT
MAX_EPISODE_STEPS=$MAX_EPISODE_STEPS_DEFAULT
N_ACTION_STEPS=$N_ACTION_STEPS_DEFAULT
...
for GPU_ID in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=${GPU_ID} \
    ${wala_PYTHON} deployment/model_server/server_policy.py \
        --ckpt_path "${CKPT_PATH}" \
        --port "${PORT}" \
        --use_bf16 \
