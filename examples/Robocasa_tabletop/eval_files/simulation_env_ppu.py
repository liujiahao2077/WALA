...
class SimulationInferenceEnv:
    ...
    @staticmethod
    def _select_vector_value(value: Any, env_idx: int, n_envs: int) -> Any:
        if value is None:
            return None
        if isinstance(value, (list, tuple)) and len(value) == n_envs:
            return value[env_idx]

        value_array = np.asarray(value)
        if value_array.ndim > 0 and value_array.shape[0] == n_envs:
            return value_array[env_idx]
        return value
    ...
    def _success_to_bool(value: Any) -> bool:
        ...
        if isinstance(value, dict):
            return SimulationInferenceEnv._success_to_bool(value.get("success", False))

        value_array = np.asarray(value)
        if value_array.dtype == object:
            return any(SimulationInferenceEnv._success_to_bool(v) for v in value_array.reshape(-1))
        if value_array.ndim == 0:
            return bool(value_array.item())
        return bool(np.any(value_array))

    @staticmethod
    def _get_env_success(env_infos: Dict[str, Any], env_idx: int, n_envs: int) -> bool:
        """Return whether this environment succeeded anywhere in the current action chunk."""
        success_info = SimulationInferenceEnv._select_vector_value(
            env_infos.get("success", False),
            env_idx,
            n_envs,
        )
        if SimulationInferenceEnv._success_to_bool(success_info):
            return True

        final_infos = env_infos.get("final_info")
        if final_infos is None:
            return False

        final_mask = env_infos.get("_final_info")
        if final_mask is not None:
            final_mask = SimulationInferenceEnv._select_vector_value(final_mask, env_idx, n_envs)
            if not SimulationInferenceEnv._success_to_bool(final_mask):
                return False

        final_info = SimulationInferenceEnv._select_vector_value(final_infos, env_idx, n_envs)
        return SimulationInferenceEnv._success_to_bool(final_info)
    ...
    def run_simulation(self, config: SimulationConfig, model: Optional[BasePolicy] = None) -> Tuple[str, List[bool]]:
        ...
        while completed_episodes < config.n_episodes:
            ...
            for env_idx in range(config.n_envs):
                current_successes[env_idx] |= self._get_env_success(env_infos, env_idx, config.n_envs)
...
def _create_single_env(config: SimulationConfig, idx: int) -> gym.Env:
    ...
    enable_render = os.getenv("ROBOCASA_ENABLE_RENDER", "0") == "1"
    ...
    record_video = os.getenv("ROBOCASA_RECORD_VIDEO", "0") == "1"
    if record_video and config.video.video_dir is not None:
...
class Args:
    ...
    #################################################################################################################
    env_name: str = (
        "gr1_unified/PnPMilkToMicrowaveClose_GR1ArmsAndWaistFourierHands_Env"  # RoboCasa task environment name
    )
    ...
    #################################################################################################################
    video_out_path: str = (
        "results/Checkpoints/<robocasa_run_id>/videos/gr1_unified/PnPMilkToMicrowaveClose_GR1ArmsAndWaistFourierHands_Env"  # Path to save videos
    )

    seed: int = 7  # Random Seed (for reproducibility)

    pretrained_path: str = (
        "results/Checkpoints/<robocasa_run_id>/final_model/pytorch_model.pt"
    )
