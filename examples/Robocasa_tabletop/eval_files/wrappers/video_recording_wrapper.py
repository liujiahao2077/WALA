...
def get_accumulate_timestamp_idxs(
    ...
    However, when overwriting previous values is desired, set last_global_idx to None.
...
class VideoRecordingWrapper(gym.Wrapper):
    def __init__(
        ...
        **kwargs,
        ...
        self.has_recorded_frames = False
    ...
    def reset(self, **kwargs):
        ...
        if self.video_dir is not None and self.file_path is not None:
            should_finalize_video = self.has_recorded_frames or self.file_path.exists()
            if should_finalize_video:
                ...
                if self.file_path.exists():
                    ...
                    print(f"[WARN] video file not found, skip rename: {self.file_path}")
    ...
    def step(self, action):
        ...
        step_success = bool(np.any(np.asarray(result[-1].get("success", False))))
        self.is_success = self.is_success or step_success
